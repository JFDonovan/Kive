...
def write_secret_key():
    ...
    '''
    with open('{}/secret_key'.format(config.app_data_path), 'wb') as secret_key_file:
...
def encrypt_response(response):
    ...
    '''
    try:
        secret_key = open('{}/secret_key'.format(config.app_data_path), 'rb').read()
...
def decrypt_request(request):
    ...
    try:
        secret_key = open('{}/secret_key'.format(config.app_data_path), 'rb').read()
        cipher = Fernet(secret_key)
        return cipher.decrypt(request.decode('utf-8'))
    except Exception as e:
        raise e